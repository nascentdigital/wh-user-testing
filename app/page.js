...
const renderPlans = (pageModel) => {
  ...
  return (
    <div 
      className='flex gap-[10px] justify-between items-center pt-[32px]' 
      key={plans.id}
    >
...
function renderPlanMenu (pageModel) {
  const {  
    packages, 
        ...
        <div className='w-full'>
          <div className='relative flex justify-center'>
            {packages.map(({id, label})=>
              renderPackageTabsButton(
                id,
                label,
                selectedPackageTab===id ,
                handlePackageTabClick,
              )
            )}
